  ...
  return (
    <div className={`table ${classes}`}>
      ...
      {rows.map(supplier => {
        ...
        return (
          <div key={supplier.id} onClick={() => clickRow(supplier)} className={`table__row${collapsedRow ? ' collapsed' : ''}${rowStatus}`}>
            {setCheckedRows &&
              <div className='table__row-item table__row-item-checkbox'>
                <Checkbox checked={checkedRows.includes(supplier.id)} onChange={(value, event) => {
            ...
            {Object.getOwnPropertyNames(supplier).map(rowItemName => {
              ...
              return (
                <div key={`${rowItemName}${supplier.id}`}
              ...
              return (
                <div key={`${action.id}`} className={`table__row-item ${action.className}`}>
                  ...
                  {!loading && ['submitted', 'in-dispute', 'not-started', 'resubmitted'].includes(supplier.status) &&
                    <Button className='outline green weekly-of-supplier__accept-action' onClick={e => supplierAction(e, 'accept-supplier')}>Accept</Button>
                  }
                  {!loading && ['submitted', 'in-dispute', 'not-started', 'resubmitted'].includes(supplier.status) &&
                    <Button className='weekly-of-supplier__dispute-action gray outline' onClick={e => supplierAction(e, 'dispute-supplier')}>Dispute</Button>
                  }
                  {!loading && ['accepted'].includes(supplier.status) &&
                    <Button className='weekly-of-supplier__revoke-action green outline' onClick={e => supplierAction(e, 'revoke-supplier')}>Revoke</Button>
                  }
            ...
            <div className='table__row-collapsed'>
                        ...
                        <div  className='table__row-collapsed-main-item'>
                          ...
                          <div className='table__row-collapsed-main-item-el'>
                            ...
                            {!loading && ['submitted', 'resubmitted', 'not-submitted'].includes(service.status) &&
                              <Button className='outline green weekly-of-supplier__accept-action' onClick={e => actionClick(e, 'accept-service')}>Accept</Button>
                            }
                            {!loading && ['submitted', 'resubmitted', 'not-submitted'].includes(service.status) &&
                              <Button className='outline gray weekly-of-supplier__dispute-action' onClick={e => actionClick(e, 'dispute-service')}>Dispute</Button>
                            }
                            {!loading && ['accepted'].includes(service.status) &&
                              <Button className='outline green weekly-of-supplier__revoke-action' onClick={e => actionClick(e, 'revoke-service')}>Revoke</Button>
                            }
                            {<ChatButton classes={!chatStatuses.includes(service.status) && 'hide'} onClick={(e) => {
                              e.stopPropagation();
                              openChat(service);
                            }} />}
                          </div>
              ...
              {!supplier.services.some(service => requestsQue.some(item => item == `${supplier.id}${service.id}`))
                ...
                <div className='table__row-collapsed-footer-item'>
                  ...
                  }} className='outline red'>Dispute all</Button>
                  <Button onClick={(e) => {
                    e.stopPropagation();
