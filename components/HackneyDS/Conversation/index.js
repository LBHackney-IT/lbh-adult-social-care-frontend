...
export default function Conversation({ conversation = [], hasDetails = true }) {
  return (
    <ul className="lbh-conversation">
      {conversation.map(({ id, inBound, message, messageFromId, createdAt, userName }) => {
        const isInBoundClass = inBound ? ' lbh-conversation__message--inbound' : '';
        return (
          <li key={id} className={`lbh-conversation__message${isInBoundClass}`}>
      ...
      {!!conversation?.length && <li className="lbh-body-xs lbh-conversation__no-older">Showing oldest messages</li>}
    </ul>
  );
}
