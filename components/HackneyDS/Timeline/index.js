...
export default function Timeline({ timelines = [] }) {
  return (
    <ol className="lbh-timeline">
      {timelines.map(({ gapBelow, actionNeeded, major, minor, header, innerElements }, index) => {
        ...
        return (
          <li className={`lbh-timeline__event${calcClasses}`} key={index}>
            {header && (
              <h3 className={header.rewriteClass || 'lbh-heading-h3'}>
                {header.link ? <Link text={header.text} noVisited /> : header.text}
              </h3>
            )}
            {innerElements?.map((element, i) => {
              if (element.text) {
                return (
                  <p key={i} className={element.rewriteClass || 'lbh-body'}>
